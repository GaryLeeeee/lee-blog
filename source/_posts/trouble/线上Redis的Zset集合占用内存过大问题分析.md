...
## 一、背景
有一天，企微收到了一条告警：Redis实例内存达到80%已自动扩容。通过分析内存变化曲线，发现是近几天才开始涨得比较快。通过大key分析拿到了几个key，命名分别是ts_file_0、ts_file_1、...、ts_file_9，很明显这些大key都是同一个场景下生成的，然后通过查看问题代码可以发现也刚好是前几天才上线的，那么问题就明确了，现在就需要来看看如何解决这些大key。（具体可参考{% post_link "redis/如何处理Redis的大key问题？" "《如何处理Redis的大key问题？》" %}）
